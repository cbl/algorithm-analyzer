...
public class Interpolation implements Algorithm<Event, Interpolation.Data> {
    ...
    public static record PartialStateEvent(
            Integer[] array, long comparisons, int left, int right, int estimate, String text)
            implements Event {
        @Override
    ...
    public void run(EventConsumer<Event> events, Data data) {
        ...
        System.out.println(
                "\n"
                    + "Yellow = left, red = estimated value, green = right. If the element is"
                    + " orange either left and estimated index are the same OR right and estimated"
                    + " index are the same.\n\n");
