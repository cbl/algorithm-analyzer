...
public class CoalescedHashTable implements HashTable
{
    ...
    public static record PartialStateEvent<T>(Value[] table, int size, int insertedKey, List<Integer> collision)
            implements Event {
        @Override
        public String toString() {
            ...
            for(int i=0;i<size;i++) {
                ...
                if(table[i].state != State.Empty) {
                    ...
                    t[2][i+1] = table[i].nextIndex != null ? Integer.toString(table[i].nextIndex) : "";
                }   
            }
